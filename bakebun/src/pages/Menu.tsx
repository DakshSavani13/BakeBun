...
const MenuItem = styled(Card)(({ theme }: { theme: Theme }) => ({
  height: '100%',
  display: 'flex',
...
// Removed unused styled sections to reduce bundle and fix lints
...
const Menu = () => {
  ...
  const calculateTotalPrice = () => {
    if (selectedPizza) {
      let basePrice = selectedPizza.price; // This is now the Medium size price
      
      // Calculate size-based pricing
      if (selectedSize === 'Small') {
        // Small size: 350-500 range (approximately 55-65% of Medium price)
        basePrice = Math.round(basePrice * 0.6);
      } else if (selectedSize === 'Large') {
        // Large size: 1000-1200 range (approximately 140-160% of Medium price)
        basePrice = Math.round(basePrice * 1.5);
      }
      // Medium size remains as base price (600-750 range)
            ...
            <DialogContent>
              <Box sx={{ mb: 3 }}>
                <FormControl component="fieldset" fullWidth>
                  ...
                  >
                    {selectedPizza.sizes.map((size: string) => (
                      <FormControlLabel
                        key={size}
                        value={size}
                        control={<Radio />}
              ...
              <Box>
                ...
                <Box sx={{ 
                  ...
                  {selectedPizza.toppings.map((topping: string) => (
                    <StyledChip
                      key={topping}
                      label={topping}
